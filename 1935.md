...
Android 上用来跑Node有两个方案，一个是直接在Android上编译安装Node，另外一个是模拟操作系统，在模拟的操作系统中进行操作。为了手机的安全(不变砖)，我使用第二种方案。

模拟其他的操作系统，我们一般使用BusyBox来搞，但是除了模拟器外，我们还需要一些对应操作系统的Bootloader脚本来启动模拟器，这里选择了一个叫做Comple的App，他整合了镜像下载，和启动脚本，并且可以自己定义脚本。
